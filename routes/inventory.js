...
// GET /api/inventory?month=2026-04
...
router.post('/', authMiddleware, async (req, res) => {
  const { month, type, meat_seafood, produce, dairy_eggs, dry_goods, beverages, other } = req.body;
  ...
  const { data, error } = await req.supabase
    ...
    .upsert({
      ...
      beverages:     Math.round((beverages     || 0) * 100),
